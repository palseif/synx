...
then
  ...
  then
    ...
    DIRECTORY=$( echo "${_SRC_UTIL}" | grep -m 1 -E $'return[[:space:]]pathRet[[:space:]]/[[:space:]](\"|\')\.' | awk '{print $4}' | sed 's/^"\(.*\)".*/\1/' )
    # '
    CONF=$( echo "${_SRC_UTIL}" | grep -F 'boost::filesystem::path pathConfigFile(GetArg("-conf",' | awk '{print $3}' | sed 's/^"\(.*\)".*/\1/' | tr ');' ' ' | sed 's/^ *//;s/ *$//' )
    if [[ -z "${CONF}" ]]
    then
      CONF=$( echo "${_SRC_UTIL}" | grep -F 'BITCOIN_CONF_FILENAME' | cut -d '=' -f2 | sed 's/^ *//;s/ *$//' | sed 's/^"\(.*\)".*/\1/' )
    fi
...
DAEMON_DOWNLOAD_EXTRACT () {
  ...
  do
    ...
    then
      ...
      if [[ $( timeout --signal=SIGKILL 1s ldd "/var/multi-masternode-data/${PROJECT_DIR}/src/${DAEMON_BIN}" | wc -l ) -gt 2 ]]
      then
        if [[ "${UBUNTU_VERSION}" == 16.* ]] && \
          [[ $( timeout --signal=SIGKILL 1s ldd "/var/multi-masternode-data/${PROJECT_DIR}/src/${DAEMON_BIN}" | grep -cE 'libboost.*1.65' ) -gt 0 ]]
        then
          echo "ldd has wrong libboost version 1.65"
          rm "/var/multi-masternode-data/${PROJECT_DIR}/src/${DAEMON_BIN}"
        elif [[ $( timeout --signal=SIGKILL 1s ldd "/var/multi-masternode-data/${PROJECT_DIR}/src/${DAEMON_BIN}" | grep -cE 'libboost.*1.54' ) -gt 0 ]]
        then
...
APPARMOR_HEADER
  ...
  then
    ...
    if [[ $( grep -c "${DAEMON_FULL}" "${APPARMOR_CONF_SYSTEMD}" ) -gt 0 ]]
    then
      ...
      then
        END=$(( START + END - 1 ))
        sed -i "${START},${END}d" "${APPARMOR_CONF_SYSTEMD}"
      fi
    fi
...
APPARMOR_DAEMON
  ...
  then
    ...
    if [[ $( grep -c "${CONTROLLER_FULL}" "${APPARMOR_CONF_SYSTEMD}" ) -gt 0 ]]
    then
...
APPARMOR_CLI
  ...
  then
    ...
    if [[ $( grep -c "${SENTINEL_FULL}" "${APPARMOR_CONF_SYSTEMD}" ) -gt 0 ]]
    then
...
SENTINEL_GENERIC_SETUP () {
  ...
  echo "Getting sentinel."
  sudo mkdir -p "${USR_HOME}/sentinel/"
  ...
  if [[ "$( sudo su "${USRNAME}" -c 'cd ~/sentinel ; venv/bin/python bin/sentinel.py' | grep -c 'Missing dependencies' )" -gt 0 ]]
  then
...
CHECK_SYSTEM () {
  ...
  CAN_SUDO=$( timeout --foreground --signal=SIGKILL 1s bash -c 'sudo -l 2>/dev/null | wc -l ' )
  if [[ ${CAN_SUDO} =~ ${RE} ]] && [[ "${CAN_SUDO}" -gt 2 ]]
  then
    :
  else
...
_masternode_dameon_2 () {
  ...
  then
    CAN_SUDO=$( timeout --foreground --signal=SIGKILL 1s bash -c 'sudo -n true >/dev/null 2>&1 && sudo -l 2>/dev/null | wc -l ' 2>/dev/null )
  fi
  ...
  (
    ...
    bash /home/mn-dropbox.sh "${1}" now "${10}" "${11}"
  ...
  (
    ...
    then
      ...
      then
        ...
        then
        (
          sudo systemctl stop "${_DAEMON_SYSTEMD_FILENAME}" 2>/dev/null
          sudo systemctl reset-failed "${_DAEMON_SYSTEMD_FILENAME}" 2>/dev/null
          sudo systemctl start "${_DAEMON_SYSTEMD_FILENAME}"
        ) &
        else
        (
          _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
          _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" start-nosystemd
        ) &
        fi
      elif [[ "$( whoami )" == "${1}" ]] && [[ ! -z "${_DAEMON_SYSTEMD_FILE}" ]]
      then
  ...
  then
  (
    if [[ ${CAN_SUDO} =~ ${RE} ]] && [[ "${CAN_SUDO}" -gt 2 ]] && [[ ! -z "${_DAEMON_SYSTEMD_FILE}" ]]
    then
        sudo systemctl restart "${_DAEMON_SYSTEMD_FILENAME}"
    else
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
      sleep 1
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" start
  ...
  (
    if [[ "${ARG10}" == 'no_wait' ]]
    then
      ( sudo systemctl stop "${_DAEMON_SYSTEMD_FILENAME}" & disown
        sudo su "${1}" -c "${CLI_BIN_LOC} stop & disown " & disown
    ...
    then
      echo "systemctl stop"
      sudo systemctl stop "${_DAEMON_SYSTEMD_FILENAME}"  >/dev/null 2>&1
    fi
    echo "${2} stop"
  ...
  (
    ...
    if [[ $( echo "${VERSION_LOCAL}" | grep -c "${VERSION_REMOTE}" ) -eq 1 ]] && [[ "${10}" != 'force' ]] && [[ "${10}" != 'force_skip_download' ]]
    then
      echo
    ...
    echo
    echo "Updating ${VERSION_LOCAL} to the latest version ${VERSION_REMOTE} for ${1}"
    echo
    TEMP_VAR_PID=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" pid )
    if [[ ! -z "${TEMP_VAR_PID}" ]]
    then
...
"
  ...
  (
    ...
    if ([ "${10}" == "remove_addnode" ] || [ "${10}" == "addnode_remove" ] || [ "${11}" == "remove_addnode" ] || [ "${11}" == "addnode_remove" ]) && [ -r "${5}" ]
    then
      echo "${5}"
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" addnode_remove
    fi
  ...
  (
    if [ "${10}" == "grep" ]
    then
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" system_log | grep -aFi "${11}"
    else
      journalctl -q -u "${1}"
    fi
  )
  ...
  (
    ...
    then
      if [[ ${CAN_SUDO} =~ ${RE} ]] && [[ "${CAN_SUDO}" -gt 2 ]]
      then
        sudo "grep" -aFi "${11}" "${DIR}/debug.log"
      else
        grep -aFi "${11}" "${DIR}/debug.log"
      fi
  ...
  (
    ...
    then
      CONF_NOW=$( grep -m 1 -E "( |^)${11}=" "${5}" | cut -d '=' -f2 )
      if [[ ! -z "${12}" ]]
      then
        ...
        then
          echo
          echo "${11} already set to ${12}."
          echo
          return
        fi

        sudo sed -i "/\(^\|\\s\)${VAR}\=/d" "${5}"
        if [[ $( echo "${12}" | tr '[:upper:]' '[:lower:]' ) == 'null' ]]
        then
          echo
          echo "Removing ${11}= from conf file."
          echo
        else
          echo
          echo "${11}=${12}" | sudo tee -a "${5}" >/dev/null
          echo "Setting ${11}=${12}."
          echo
        fi
        _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
      ...
      then
        echo
        echo "${11} is currently set to ${CONF_NOW}"
        echo

      elif [[ -z "${CONF_NOW}" ]]
      then
        echo
        echo "${11} is currently not set."
        echo
      fi
    else
      sudo cat "${5}"
  ...
  (
    ...
    then
      ...
      if [[ "${PORT_BIND}" == "${10}" ]] && [[ "${PORT_EXTERNAL}" == "${10}" ]]
      then
        echo
        echo "Port already set to ${10}."
        echo
        return
      fi

      if [[ "$( sudo netstat -tulpnW | grep -cF "${IP_BIND}:${10}" )" -gt 0 ]]
      then
        echo
        echo "Port already in use; netstat failure."
        sudo netstat -tulpnW | grep -F "${IP_BIND}:${10}"
        echo
        return
      fi
      NETCAT_TEST=$( sudo timeout --signal=SIGKILL 0.2s netcat -p "${10}" -l "${IP_BIND}" 2>&1 )
      NETCAT_PID=$!
      kill -9 "${NETCAT_PID}" >/dev/null 2>&1
      ...
      echo "externalip=${IP_EXTERNAL}:${10}" | sudo tee -a "${5}" >/dev/null
      echo "bind=${IP_BIND}:${10}" | sudo tee -a "${5}" >/dev/null
      # Open up port.
      sudo ufw allow "${10}" >/dev/null 2>&1
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
      sleep 1
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" start
    fi
  ...
  (
    ...
    then
      ...
      then
        ...
        _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}.conf"
      else
        echo
        echo "${BALANCE}"
  ...
  elif [ "${ARG9}" == "masternode.conf" ] || [ "${ARG9}" == "${_MASTERNODE_CALLER}.conf" ]
  then
  (
    if [[ "${10}" == 'loc' ]] || [[ "${10}" == 'location' ]]
    then
      touch "${DATADIR}/${_MASTERNODE_CALLER}.conf"
      chown "${1}":"${1}" "${DATADIR}/${_MASTERNODE_CALLER}.conf"
      echo "${DATADIR}/${_MASTERNODE_CALLER}.conf"

    elif [[ "${10}" == 'nano' ]]
    then
      nano "${DATADIR}/${_MASTERNODE_CALLER}.conf"
      chown "${1}":"${1}" "${DATADIR}/${_MASTERNODE_CALLER}.conf"
    ...
    then
      touch "${DATADIR}/${_MASTERNODE_CALLER}.conf"
      chown "${1}":"${1}" "${DATADIR}/${_MASTERNODE_CALLER}.conf"
      cat "${DATADIR}/${_MASTERNODE_CALLER}.conf"
    ...
    then
      ...
      then
        touch "${DATADIR}/${_MASTERNODE_CALLER}.conf"
        echo "${1}_${PART_A} ${PART_B1} ${PART_C} ${PART_D} ${PART_E}" >> "${DATADIR}/${_MASTERNODE_CALLER}.conf"
        chown "${1}":"${1}" "${DATADIR}/${_MASTERNODE_CALLER}.conf"
      fi
    fi
  )
...
${TEMP_FILE}
  ...
  elif [ "${ARG9}" == "masternodeping" ] || [ "${ARG9}" == "mnping" ] || [ "${ARG9}" == "${_MASTERNODE_CALLER}ping" ] || [ "${ARG9}" == "${_MASTERNODE_PREFIX}ping" ]
  then
  (
    DATE_STRING=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" daemon_log tac | grep -ai -m1 "active.*${_MASTERNODE_CALLER}ping" | awk '{print $1 " " $2}' )
    if [[ ! -z "${DATE_STRING}" ]]
    then
  ...
  (
    ...
    then
      ...
      echo "already running. PID ${PID_TO_KILL}"

      if [[ "$( ps -o etimes= -p "${PID_TO_KILL}" | awk '{ print $1}' )" -gt 100000 ]]
      then
        echo "Killing old mnfix call."
        kill "${PID_TO_KILL}"
        kill -9 "${PID_TO_KILL}"
      else
        return 1 2>/dev/null
    ...
    then
      echo "System overloaded; skipping mnfix."
      return 1 2>/dev/null
    fi
    ...
    then
      echo "Restarting ${1} as it is frozen."
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
      sleep 2
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
      sleep 2
    ...
    then
      ...
      then
        ...
        then
          ...
          sudo systemctl enable "${_DAEMON_SYSTEMD_FILENAME}" 2>&1
        fi
      ...
      if [[ $( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}ping" ) -gt 1500 ]]
      then
        echo "Restarting ${1} because mn hasn't pinged the network in over 1500 seconds."
        _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
        sleep 2
        _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" start
        _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}ping"
      fi
    fi
    ...
    then
      ...
      if [[ ! -z "${DROPBOX_BLOCKS_N_CHAINS}" ]]
      then
        echo "Using blocks snapshot."
        _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" dl_blocks_n_chains force
      elif [ ! -z "${DROPBOX_BOOTSTRAP}" ]
      then
        echo "Using bootstrap file."
    ...
    then
      ...
      then
        ...
        then
          echo "Blockcount has not moved in over ${LASTBLOCK_TIME} seconds and is not the same as the explorers."
  ...
  (
    ...
    if [[ $( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" conf | grep -c "${_MASTERNODE_CALLER}=1" ) -lt 1 ]]
    then
      echo "ERROR: ${_MASTERNODE_CALLER} ${1} is not conifgured to be a ${_MASTERNODE_CALLER} (missing ${_MASTERNODE_CALLER}=1)."
    fi

    if [[ $( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" conf | grep -c "${_MASTERNODE_CALLER}privkey=" ) -lt 1 ]]
    then
    ...
    MN_PING_TIME=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}ping" )
    ...
    then
      ...
      WEB_BLK_HIGH=$(( WEB_BLK + EXPLORER_BLOCKCOUNT_OFFSET ))
      WEB_BLK_LOW=$(( WEB_BLK - EXPLORER_BLOCKCOUNT_OFFSET ))
      echo "WARNING: ${_MASTERNODE_CALLER} ${1} blockcount is not correct. Local Count:${LOCAL_BLK}, Network Count:${PEER_BLK}."
  ...
  (
    ...
    _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" stop
    _NEW_CRONTAB=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}"  crontab -l | sed "s/${1} /${10} /g" )
    ...
    then
      ...
      sudo sed -i "s/\\/${1}\\//\\/${10}\\//g" /etc/apparmor.d/multi-masternode-data
      sudo systemctl reload apparmor.service
    fi
    ...
    if [[ ! -z "${_NEW_CRONTAB}" ]]
    then
      sudo su "${10}" -c " echo \"${_NEW_CRONTAB}\" | crontab - "
    fi
    echo
  )
  unset -f "${1}"
  ...
  elif [ "${ARG9}" == "getmasternodever" ] || [ "${9}" == "mnver" ] || [ "${9}" == "getmasternodeversion" ] || [ "${9}" == "masternodever" ] || [ "${9}" == "get${_MASTERNODE_CALLER}ver" ] || [ "${9}" == "get${_MASTERNODE_CALLER}version" ] || [ "${9}" == "${_MASTERNODE_CALLER}ver" ] || [ "${9}" == "${_MASTERNODE_PREFIX}ver" ]
  then
  (
    _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}" list |  jq '.[] | "\(.version)"' | sort -hr | uniq -c
  )
  ...
  (
    ...
    then
      ...
      do
        ...
        then
          aria2c --console-log-level=warn -x 4 "https://www.dropbox.com/s/${DROPBOX_BOOTSTRAP}/bootstrap.dat.gz?dl=1" -d /tmp -o "${PROJECT_DIR}.bootstrap.dat.gz"
        else
          wget -4 "https://www.dropbox.com/s/${DROPBOX_BOOTSTRAP}/bootstrap.dat.gz?dl=1" -O "/tmp/${PROJECT_DIR}.bootstrap.dat.gz" -q --show-progress --progress=bar:force 2>&1
        fi
        sleep 0.6
        echo
        echo "Decompressing..."
    ...
    then
      sleep 5
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" reindex "${10}" "${11}"
    elif [[ ! -z "${TEMP_VAR_PID}" ]]
    then
      sleep 5
      _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" start
  ...
  (
    ...
    then
      ...
      do
        ...
        if [[ "/var/multi-masternode-data/${PROJECT_DIR}/blocks_n_chains" != "${BASE_FOLDER}" ]]
        then
    ...
    then
      ...
      find "${DIR}" -maxdepth 1 | tail -n +2 | grep -vE "backups|wallet.dat|${FILENAME}|${_MASTERNODE_CALLER}.conf|peers.dat|debug.log" | xargs rm -rf
    fi
...
${TXID} ${OUTPUTIDX}"
  ...
  (
    ...
    then
      ...
      sleep 0.1
      MASTERNODE_STATUS=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}" status )
      if [[ $( echo "${MASTERNODE_STATUS}" | grep -ic "${_MASTERNODE_CALLER} successfully started" ) -ge 1 ]] || [[ $( echo "${MASTERNODE_STATUS}" | grep -ic "${_MASTERNODE_CALLER} started remotely" ) -ge 1 ]]
      then
        # Get collateral info.
    ...
    then
      ...
      sleep 0.3
      MASTERNODE_LIST=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}" list )
      ...
      then
        sleep 0.4
        if [[ $( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}" list help | grep -ic 'json' ) -gt 0 ]]
        then
          sleep 0.5
          MASTERNODE_LIST_JSON=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}" list json )
      ...
      then
        sleep 0.6
        MASTERNODE_LIST_ALL=$( _masternode_dameon_2 "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "${_MASTERNODE_CALLER}" list full )
...
${LOG_LINES}"
  ...
  (
    if [[ ${CAN_SUDO} =~ ${RE} ]] && [[ "${CAN_SUDO}" -gt 2 ]]
    then
      JSON_STRING=$( sudo su "${1}" -c "${CLI_BIN_LOC} -datadir=${DATADIR}/ ${ARG9} ${10} ${11} ${12} ${13} ${14} ${15} ${16} ${17} " 2>&1 )
    else
      # shellcheck disable=SC2086
      JSON_STRING=$( "${CLI_BIN_LOC}" "-datadir=${DATADIR}/" "${ARG9}" ${10} ${11} ${12} ${13} ${14} ${15} ${16} ${17} 2>&1 )
    fi
...
all_mn_run () {
  ...
  done <<< "$( cut -d: -f1 /etc/passwd | getent passwd | cut -d: -f6 )"
}
...
UPDATE_DAEMON_ADD_CRON () {
  ...
  do
    ...
    MN_USRNAME=$( basename "${USR_HOME_DIR}" )
    if [ "$( type "${MN_USRNAME}" 2>/dev/null | grep -c '_masternode_dameon_2' )" -gt 0 ] || [[ $( "${MN_USRNAME}" daemon ) != "${DAEMON_BIN}" ]]
    then
      continue
    fi
    ...
    if [[ $( grep -c 'masternode_caller=' < "${CONF_FILE}" ) -gt 0 ]]
    then
      _MASTERNODE_CALLER=$( grep -m 1 'masternode_caller=' "${CONF_FILE}" | cut -d '=' -f2 )
    fi
    ...
    then
      ...
      sudo su "${MN_USRNAME}" -c " ( crontab -l ; echo \"${MINUTES},${MINUTES_A},${MINUTES_B} * * * * bash -ic 'source /var/multi-masternode-data/.bashrc; ${MN_USRNAME} mnfix 2>&1' 2>/dev/null\" ) | crontab - "
    fi
    ...
    "${MN_USRNAME}" wait_for_loaded "${_MASTERNODE_CALLER}"
...
then
  ...
  do
    ...
    do
      ...
      OUTPUTIDX_WEB=$( echo "${OUTPUTIDX_RAW}" | tr '[:upper:]' '[:lower:]' | jq ".vout[] | select( (.value)|tonumber == ${COLLATERAL_ADJUSTED} ) | .n" )
...
do
  ...
  then
    ...
    echo "This linux box may not have enough resources to run another ${MASTERNODE_CALLER} daemon."
...
then
  ...
  "${USRNAME}" wait_for_loaded "${MASTERNODE_CALLER}"
...
then
  ...
  sleep 1
  echo "Adding ${MASTERNODE_CALLER} key to ${USRNAME} configuration"
...
then
  ...
  MINUTES_A=$(( MINUTES + 20 ))
  MINUTES_B=$(( MINUTES + 40 ))
  sudo su "${USRNAME}" -c " ( crontab -l ; echo \"${MINUTES},${MINUTES_A},${MINUTES_B} * * * * bash -ic 'source /var/multi-masternode-data/.bashrc; ${USRNAME} mnfix 2>&1' 2>/dev/null\" ) | crontab - "
fi
...
RUNNING_DAEMON_PORTS=$( sudo lslocks | tail -n +2 | awk '{print "pid=" $2}' | sort -u )
...
then
  ...
  done <<< "$( sudo lslocks | tail -n +2 | awk '{print $2 "/"}' | sort -hu )"
  echo
...
if [[ $( "${USRNAME}" help | awk '{print $1}' | grep -c "start${MASTERNODE_CALLER}" ) -gt 0 ]]
then
    ...
    then
      ...
      then
        ...
        then
          ...
          "${USRNAME}" "${MASTERNODE_CALLER}" debug
          sleep 0.5
          "${USRNAME}" "${MASTERNODE_PREFIX}info"
          echo
